...
class ELMRegression:
    def __init__(self, hidden_nodes, save_path, is_standard=False, Dimensionality_reduction_method='None'):
        # num_hidden 隐藏层神经元个数
    ...
    def score(self, x_test, y_test):
        ...
        self.r2, self.r2_adjusted, self.rmsle = tools.reg_calculate(y_test, self.prediction, features=x_test.shape[-1])
    ...
    def result_plot(self, save_file):

        plt.plot(range(len(self.prediction)), self.prediction, 'r--', label='prediction')
    ...
    def save(self):
        ...
        count = tools.save_elm(self.hidden_nodes, self.mse, self.rmse, self.mae, self.mad, self.mape,
        ...
        self.result_plot(save_file=pic_file)
