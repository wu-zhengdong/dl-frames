...
class ANN():
    ...
    def __save_result(self, save_path):
        ...
        try:
            ...
            lr = self.lr
        count = tools.save_ann_results(self.e, self.batch_size, lr, self.dropout,
                                       layer_numbers, hidden_layers, self.activate_function, self.weight_decay,
                                       self.mse, self.rmse,
                                       self.mae, self.mad, self.mape, self.r2, self.r2_adjusted, self.rmsle, self.is_standard,
...
class CNN(object):
    ...
    def __save_result(self, save_path):
        ...
        try:
            ...
            lr = self.lr
        count = tools.save_cnn_results(self.e, self.batch_size, lr, self.dropout, layer_numbers, hidden_layers,
                               self.kernel_size, self.conv_stride, self.pooling_size, self.pool_stride, self.flatten,
                               self.activate_function, self.weight_decay, self.mse, self.rmse, self.mae, self.mad,
                               self.mape, self.r2,
                               self.r2_adjusted, self.rmsle, self.is_standard, self.t,
                               self.Dimensionality_reduction_method, save_path)
        print('Save results success!')
        return count
...
class LSTM():
    ...
    def __save_result(self, save_path):
        ...
        try:
            ...
            lr = self.lr
        count = tools.save_lstm_results(self.e, self.batch_size, lr, self.dropout, self.num_layers, self.hidden_size,
                                self.activate_function, self.weight_decay, self.mse, self.rmse, self.mae,
                                self.mad, self.mape, self.r2,
